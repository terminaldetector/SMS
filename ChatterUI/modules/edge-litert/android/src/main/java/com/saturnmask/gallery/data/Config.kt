...
import expo.modules.edgelitert.R
...
object ConfigKeys {
  ...
  val NUM_THREADS = ConfigKey("num_threads", "CPU threads", R.string.config_label_num_threads)
  val SEED = ConfigKey("seed", "Seed", R.string.config_label_seed)
...
fun createLlmChatConfigs(
  ...
  val configs =
    listOf(
        ...
        NumberSliderConfig(
          key = ConfigKeys.NUM_THREADS,
          sliderMin = 0f, // 0 = let the native engine pick its own default; CPU backend only.
          sliderMax = 8f,
          defaultValue = defaultNumThreads.toFloat(),
          valueType = ValueType.INT,
        ),
        NumberSliderConfig(
          key = ConfigKeys.SEED,
          sliderMin = 0f,
          sliderMax = 999999f,
          defaultValue = defaultSeed.toFloat(),
          valueType = ValueType.INT,
        ),
