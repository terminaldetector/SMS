...
Главный узел:
```bash
./build/bin/llama-cli -m model-Q4_K_M.gguf -ngl 33 \
    --rpc 192.168.49.10:50052,192.168.49.11:50052 \
    --tensor-split 0.6,0.4 -p "..."
```

**Что здесь легко понять неправильно** (и это уже ломало реальный меш — см.
`helix.rpc_cluster.llama_rpc_args`):
- `--tensor-split` индексируется **списком устройств** llama.cpp, а это `[rpc-устройства…,
  локальные GPU…]`. CPU главного узла в этот список **не входит**, и доли «для себя» в нём нет:
  долей ровно столько, сколько rpc-устройств. Отдать сюда доли на всё кольцо — сдвинуть каждому
  воркеру чужую долю (а при одном воркере отдать ему весь хвост).
- `-ngl` — это **сколько последних слоёв разрешено уйти** с главного узла (плюс output-слой, он
  всегда едет с хвостом), а не сколько слоёв в модели. `-ngl 99` = «главный не держит ничего,
  воркеров просят целиком всю модель».
- Значит: главный держит первую полосу на своём CPU, `-ngl` = размер удалённого хвоста + 1, а
  доли считаются **по воркерам** и суммируются в 1.
...
3. **Сформировать запуск:** `helix.rpc_cluster.llama_rpc_args(plan)` → `--rpc <воркеры кольца>`,
   `--tensor-split <доли по воркерам>`, `-ngl <хвост+1>`; поднять `rpc-server` на воркерах.
   Готовые команды печатает `python -m helix.host.rpc_launch`.
...
3. Запустить главный: `llama-cli -m 16b-Q4_K_M.gguf -ngl <хвост+1> --rpc <воркер:50052> -ts
   <доли по воркерам>` — обе величины берутся из `llama_rpc_args`, не из полей плана.
...
- Разошёлся `--tensor-split` vs реальная память → OOM: доверять **аттестованной** ёмкости, не
  заявленной.
- `--tensor-split`/`-ngl` в терминах кольца, а не устройств llama.cpp → шард «загрузился и
  отвечает», но работает один телефон. Проверять не план, а `devices` из результата загрузки:
  если в нём нет ни одного `RPC*`, слои никуда не уехали.
