...
export type NativeContextParams = {
  ...
  /**
   * List of `ggml-rpc` server endpoints (`"host:port"`) to register as remote backend
   * devices before model load, e.g. for offloading layers to another device on the LAN.
   * Requires the native module to be built with `LM_GGML_USE_RPC` (Android only for now).
   */
