...
#if defined(LM_GGML_USE_RPC)
    // Endpoints already handed to lm_ggml_backend_rpc_add_server() so far. Each call registers a
    // new "RPC" device with the global backend registry; re-registering the same endpoint on every
    // context init would grow the registry unboundedly, so track what's already been added.
    static std::set<std::string>& registeredRpcServers() {
