...
bool socket_t::impl::rdma_probe() {
    const char * dev_env = std::getenv("LM_GGML_RDMA_DEV");
    const char * gid_env = std::getenv("LM_GGML_RDMA_GID");
    ...
    for (int d = 0; d < num_devs; d++) {
        ...
        if (found_gid < 0) {
            ...
            // Explicit GID index from LM_GGML_RDMA_GID — fetch its type for logging.
            ibv_gid_entry entry = {};
    ...
    LM_GGML_LOG_INFO("RDMA probed: dev=%s gid=%d%s qpn=%u inline=%u\n",
...
bool socket_t::impl::rdma_activate(uint32_t remote_qpn, uint32_t remote_psn, const uint8_t * remote_gid) {
    ...
    LM_GGML_LOG_INFO("RDMA activated: qpn=%u->%u mtu=%d rx_depth=%d\n",
    ...
    for (uint64_t s = 0; ; s++) {
        ...
        if (n > 0) {
            if (wc->status != IBV_WC_SUCCESS) {
                LM_GGML_LOG_ERROR("RDMA CQ wc error: status=%d (%s) vendor_err=0x%x\n",
...
void socket_t::impl::update_caps(const uint8_t * remote_caps) {
    ...
    if (rdma_activate(rc.qpn, rc.psn, rc.gid)) {
        ...
        LM_GGML_LOG_ERROR("RDMA activate failed, staying on TCP\n");
        rdma.reset();
    }
#else
    (void)remote_caps;
