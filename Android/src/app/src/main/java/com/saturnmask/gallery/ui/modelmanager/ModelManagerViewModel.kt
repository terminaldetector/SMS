...
import com.saturnmask.gallery.common.isAICoreSupported
...
import com.saturnmask.gallery.data.EMPTY_MODEL
import com.saturnmask.gallery.data.IMPORTS_DIR
import com.saturnmask.gallery.data.Model
import com.saturnmask.gallery.data.ModelAllowlist
import com.saturnmask.gallery.data.ModelCapability
import com.saturnmask.gallery.data.ModelCapabilityOverrideStore
...
import com.saturnmask.gallery.data.SOC
import com.saturnmask.gallery.data.SystemPromptRepository
import com.saturnmask.gallery.data.TMP_FILE_EXT
import com.saturnmask.gallery.data.Task
import com.saturnmask.gallery.data.createLlmChatConfigs
import com.saturnmask.gallery.proto.AccessTokenData
...
private val PREDEFINED_LLM_TASK_ORDER = listOf(BuiltInTaskId.LLM_AGENT_CHAT)
...
constructor(
  ...
  private val modelCapabilityOverrideStore: ModelCapabilityOverrideStore,
  ...
  fun deleteModel(model: Model, removeImportedFromModelList: Boolean = true) {
    ...
    if (model.imported && removeImportedFromModelList) {
      ...
      // A reimport under this filename may be a different file — don't let a stale
      // vision/audio-support override from the deleted one carry over.
      modelCapabilityOverrideStore.clearOverride(model)
  ...
  fun addImportedLlmModel(info: ImportedModel) {
    ...
    val setOfTasks = mutableSetOf(BuiltInTaskId.LLM_AGENT_CHAT, BuiltInTaskId.LLM_CODER)
    for (task in getTasksByIds(ids = setOfTasks)) {
      ...
      if (modelIndex >= 0) {
        ...
        // A same-filename reimport may be a different file — an override recorded for the old
        // one shouldn't carry over. Key only depends on name/downloadFileName/version (identical
        // between old and new imported-model instances sharing a filename), so this is safe to
        // call with the newly-created `model` regardless of which instance triggered it.
        modelCapabilityOverrideStore.clearOverride(model)
      }
    ...
    viewModelScope.launch(Dispatchers.IO) {
      try {
        ...
        for (allowedModel in modelAllowlist.models) {
          ...
          modelCapabilityOverrideStore.applyOverride(model)
  ...
  private fun createUiState(): ModelManagerUiState {
    ...
    for (importedModel in dataStoreRepository.readImportedModels()) {
      ...
      tasks.get(key = BuiltInTaskId.LLM_CODER)?.models?.add(model)
  ...
  private fun createModelFromImportedModelInfo(info: ImportedModel): Model {
    ...
    // BUGFIX: manually-imported models (via the HF URL import flow) have no source for a true
    // per-model max context length — LlmConfig proto only carries `default_max_tokens`, a single
    // recommended value, not a hardware/model ceiling. Without a non-null
    // `defaultMaxContextLength`, createLlmChatConfigs() renders MAX_TOKENS as a read-only
    // LabelConfig instead of an editable NumberSliderConfig (see its `if
    // (defaultMaxContextLength != null)` branch) — so context length was silently
    // uneditable for every imported model, regardless of which chat screen you're in.
    // 32768 is a fixed, generous ceiling (not model-specific) — Gemma 4's own model card lists
    // "up to 32k context length" for the E2B/E4B sizes, so this is a reasonable universal
    // default rather than an arbitrary number, but it is still a guess for models whose real
    // limit differs. Coerced against llmMaxToken so the slider's default is never above its max.
    val importedModelMaxContextLengthCeiling = 32768
    val configs: MutableList<Config> =
      createLlmChatConfigs(
          defaultMaxToken = llmMaxToken,
          defaultMaxContextLength = maxOf(llmMaxToken, importedModelMaxContextLengthCeiling),
    ...
    if (llmSupportThinking) {
      ...
      capabilityToTaskTypes[ModelCapability.LLM_THINKING] = listOf(BuiltInTaskId.LLM_AGENT_CHAT)
    }
