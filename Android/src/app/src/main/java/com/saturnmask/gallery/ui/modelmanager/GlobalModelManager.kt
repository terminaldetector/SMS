...
import com.saturnmask.gallery.ui.common.buildTrackableUrlAnnotatedString
...
fun GlobalModelManager(
  ...
  // AgentChatTask (LLM_AGENT_CHAT) is the only registered CustomTask left, so a model is never in
  // more than one task's model list — no "which use case?" picker needed, just go straight there.
  val handleClickModel: (Model) -> Unit = { model ->
    val tasks = viewModel.uiState.value.tasks
    val task = tasks.firstOrNull { task -> task.models.any { it.name == model.name } }
    if (task != null) {
