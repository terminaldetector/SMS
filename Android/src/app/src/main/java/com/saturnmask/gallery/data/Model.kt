...
data class Model(
   ...
   * "External Files Directory": /storage/emulated/0/Android/data/com.saturnmask.gallery/files/.
   *
   * For example, if this field is set to "my_model/local_dir/", then the location you should push
   * files to is:
   *
   * /storage/emulated/0/Android/data/com.saturnmask.gallery/files/my_model/local_dir/
   *
   ...
   *
   * Declared up front (allowlist JSON, or a manual import toggle), but not authoritative — the
   * model's .task/.litertlm file is the actual source of truth. LlmChatModelHelper.initialize()
   * corrects this in place the first time it discovers a mismatch (engine creation fails
   * requesting a vision encoder that isn't in the file), so later loads and UI (attach-image
   * button visibility) reflect what the model file actually contains.
   */
  var llmSupportImage: Boolean = false,

  /** Whether the LLM model supports audio input. Corrected the same way as [llmSupportImage]. */
  var llmSupportAudio: Boolean = false,
