...
import com.saturnmask.gallery.common.isPixelDevice
...
data class AllowedModel(
  ...
  fun toModel(): Model {
    ...
    // Config.
    // "llm_chat" is read here purely as a data tag, not as a live Task lookup key — no Task with
    // that id is registered anymore (the standalone "AI Chat" screen was deleted; AgentChat is
    // the only chat entry point now). Checked model_allowlists/1_0_15.json (the current live
    // allowlist, fetched by BuildConfig.VERSION_NAME at runtime): every general-purpose model
    // still carries "llm_chat" in taskTypes, but only 2 of 8 (the Gemma 4 models) carry
    // "llm_agent_chat" — so gating isLlmModel on llm_agent_chat alone would have wrongly
    // declared Gemma-3n/Gemma3-1B/Qwen/DeepSeek as non-LLM models. "llm_chat" is the one tag
    // that's universal across every general model and absent from the narrow single-purpose
    // finetunes (TinyGarden-270M, MobileActions-270M carry only their own dedicated tag) — so it
    // stays as the isLlmModel marker. BuiltInTaskId.LLM_CHAT is kept for exactly this reason (see
    // its doc comment in Tasks.kt).
