...
| **2. Agent** | его модель = **Track A** агент (`completion`→`AgentRunner`) | 🟡 среднее | маппинг готов; нужен **TS-порт** HELIX-клиента (codec+transport+worker), сверка с `vectors.json` |
...
**Вывод:** Уровень 1 почти готов; Уровень 2 спроектирован (нужен небольшой TS-порт); Уровень 3 —
только дизайн (нативная работа).
